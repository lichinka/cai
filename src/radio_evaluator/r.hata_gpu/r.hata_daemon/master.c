                                ...
                                const int uncovered_coord_length)
{
    ...
    for (i = 0; i < 1000; i++) 
    {    
        rnd_idx = rand( ) % uncovered_coord_length - 1; 
...
/**
 * Executes the agents kernel for optimization.
 *
                        ...
                        const int ntx)
{
    ...
    printf ("Launching agent kernel ...\n");

    init_agent_area (agent_area, pl, ntx);

    // load, compile and link the OpenCL kernel file
    void* h = clopen (ctx, NULL, CLLD_NOW);

    // select the correct kernel
    cl_kernel krn = clsym (ctx, h, "agent_kern", CLLD_NOW);

    // thread arranged in a 1D grid
    ...
    clarg_set (ctx, krn, 2, mdim);
    clarg_set (ctx, krn, 3, cov_info.uncovered_count);
    clarg_set_global (ctx, krn, 4, pl_in);
    clarg_set_global (ctx, krn, 5, offsets_in);
    clarg_set_global (ctx, krn, 6, qrm_in);
    clarg_set_global (ctx, krn, 7, cov_in);
    clarg_set_global (ctx, krn, 8, uncov_coord_in);
    clarg_set_global (ctx, krn, 9, tx_pwr);
    ...
    clarg_set_local (ctx, krn, 10, lmem_size);

    for (i=0; i<ntx; i++)
        printf ("\t%d. transmitter power is %d\n", i, tx_pwr[i]);

    // start kernel execution 
    clfork (ctx, 0, krn, &ndr, CL_EVENT_WAIT);
    ...
    if (strcmp (opt7->answer, "yes") == 0)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                int *pilots = init_pilots (ctx,
                                           ntest_tx, 
                                           _HATA_INITIAL_PILOT_POWER_);
